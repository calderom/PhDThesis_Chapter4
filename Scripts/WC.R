...
c <- ggplot(LakeWC, aes(x = Date, y = colour.mgPtL))+ 
  geom_line()+
  geom_point(shape=21, color="black", fill="yellow", size=4)+
...
d <- ggplot(LakeWC, aes(x = Date, y = SUVA254.AULmmgC))+ 
  geom_line()+
  geom_point(shape=21, color="black", fill="gold2", size=4)+
...
e <- ggplot(LakeWC, aes(x = Date, y = chla.ugL))+ 
  geom_line()+
  geom_point(shape=21, color="black", fill="greenyellow", size=4)+
...
f <- ggplot(LakeWC, aes(x = Date, y = TSS.mgL))+ 
  geom_line()+
  geom_point(shape=21, color="black", fill="chocolate", size=4)+
...
g <- ggplot(NitrogenWC, aes(x = Date, y = Value, group=Variable))+ 
  geom_line()+
  geom_point(aes(fill=Variable), shape = 21, color="black", size=4)+
...
i <- ggplot(LakeWC, aes(x = Date, y = dissSi.mgL))+ 
  geom_line()+
  geom_point( shape=21, color="black", fill="purple", size=4)+
...
j <- ggplot(LakeWC, aes(x = Date, y = Si.TP.molar))+ 
  geom_line()+
  geom_point( shape=21, color="black", fill="violetred", size=4)+
  ylab("Si:TP (molar)")+
  scale_x_date(breaks=date_breaks("1 month"),labels=date_format("%b")) +
  theme(panel.background = element_rect(fill = 'white', colour = 'black'),
        text = element_text(size = 10), axis.title.x = element_blank())
